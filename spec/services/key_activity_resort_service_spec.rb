...
RSpec.describe KeyActivityResortService do
  ...
  describe "call" do
    ...
    it "resorts items by due date and priority if selected to do so" do
      ...
      expect(KeyActivity.where(scheduled_group: scheduled_group_today.id).sort_by_due_date_and_priority).to eq([ka4, ka1, ka2, ka3, ka0])
      expect(KeyActivity.where(scheduled_group: scheduled_group_tomorrow.id).sort_by_due_date_and_priority).to eq([kat1, kat0])
    end
  end
end